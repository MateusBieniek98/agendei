...
function maintenanceDays(start: string) {
  const startTime = new Date(start).getTime();
  if (!Number.isFinite(startTime)) return 0;
  return Math.max(0, Math.floor((Date.now() - startTime) / 86_400_000));
}
...
export default function MaintenanceMachines() {
  ...
  return (
    <section className="space-y-5 pb-8">
      ...
      ) : <div className="ui-list-enter grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
        {visible.map((machine) => {
          ...
          const current = threads.find((item) => item.maquina_id === machine.id && item.status !== "resolvido");
          return <button key={machine.id} type="button" onClick={() => openMachine(machine.id)} className="rounded-lg border border-[var(--border)] bg-[var(--bg-card)] p-4 text-left transition-colors hover:border-[var(--border-strong)] hover:bg-[var(--bg-hover)]"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><Badge tone={meta.tone}>{meta.label}</Badge><h2 className="mt-2 truncate text-base font-semibold text-[var(--text-primary)]">{machine.nome}</h2><p className="truncate text-xs font-normal text-[var(--text-muted)]">{machine.tipo}{machine.identificador ? ` · ${machine.identificador}` : ""}</p>{open > 0 && <p className="mt-2 text-xs font-medium text-[var(--danger)]">Parada há {maintenanceDays(current?.parada_desde ?? current?.created_at ?? "")} dias</p>}{current && <p className="mt-1 line-clamp-2 text-xs text-[var(--text-secondary)]">{current.situacao_atual}</p>}</div><span className="text-xl text-[var(--text-muted)]">›</span></div></button>;
        })}
      </div>}
      ...
      {selected && <div className="ui-overlay fixed inset-0 z-[70] bg-black/35" data-state="open" data-side="right" onClick={() => setSelectedId(null)}><aside className="ui-drawer-panel ml-auto h-full w-full overflow-y-auto bg-[var(--bg-page)] shadow-xl sm:max-w-xl" onClick={(event) => event.stopPropagation()}><div className="sticky top-0 z-10 flex items-center justify-between border-b border-[var(--border)] bg-[var(--bg-card)] px-4 py-3"><div className="min-w-0"><p className="text-xs font-bold uppercase text-[var(--text-muted)]">Máquina</p><h2 className="truncate text-lg font-bold text-[var(--text-primary)]">{selected.nome}</h2></div><Button variant="ghost" size="sm" onClick={() => setSelectedId(null)}>Fechar</Button></div><div className="space-y-4 p-4 sm:p-5"><div className="rounded-lg border border-[var(--border)] bg-[var(--bg-card)] p-4"><p className="text-sm font-semibold text-[var(--text-secondary)]">{selected.tipo}{selected.identificador ? ` · ${selected.identificador}` : ""}</p><div className="mt-4"><Select label="Status operacional" value={selected.status} disabled={saving} onChange={(event) => void updateStatus(selected, event.target.value as MachineStatus)} options={STATUS_OPTIONS} /></div></div><div className="rounded-lg border border-[var(--border)] bg-[var(--bg-card)] p-4"><h3 className="text-sm font-bold text-[var(--text-primary)]">Chamados pendentes</h3>{pendingThreads.length === 0 ? <p className="mt-3 text-sm font-semibold text-[var(--text-muted)]">Nenhum chamado pendente.</p> : <div className="mt-3 space-y-3">{pendingThreads.map((thread) => <div key={thread.id} className="border-l-2 border-[var(--border)] pl-3"><div className="flex flex-wrap gap-1"><Badge tone="warning">Parada há {maintenanceDays(thread.parada_desde ?? thread.created_at)} dias</Badge><Badge tone={thread.prioridade === "urgente" ? "danger" : thread.prioridade === "alta" ? "warning" : "neutral"}>{thread.prioridade}</Badge></div><p className="mt-2 text-sm font-semibold text-[var(--text-primary)]">{thread.situacao_atual}</p><p className="mt-1 text-xs text-[var(--text-secondary)]">{thread.descricao}</p><p className="mt-1 text-[11px] text-[var(--text-muted)]">{ddmmyyyy(thread.created_at)}{thread.responsavel ? ` · ${thread.responsavel.nome}` : ""}</p></div>)}</div>}</div><div className="rounded-lg border border-[var(--border)] bg-[var(--bg-card)] p-4"><h3 className="text-sm font-bold text-[var(--text-primary)]">Histórico</h3>{events.length === 0 ? <p className="mt-3 text-sm font-semibold text-[var(--text-muted)]">O histórico começa nas próximas alterações.</p> : <div className="mt-3 space-y-3">{events.map((event) => <div key={event.id} className="border-l-2 border-[var(--border)] pl-3"><p className="text-sm font-bold text-[var(--text-primary)]">{EVENT_LABEL[event.tipo]}{event.tipo === "situacao_atualizada" && event.dados.novo ? ` · ${String(event.dados.novo)}` : ""}</p><p className="mt-1 text-[11px] text-[var(--text-muted)]">{ddmmyyyy(event.created_at)}{event.ator?.nome ? ` · ${event.ator.nome}` : ""}</p></div>)}</div>}</div></div></aside></div>}
    </section>
  );
}
