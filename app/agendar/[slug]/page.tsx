...
export default function AgendarPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  async function loadProf() {
    try {
      ...
      const { data: byId } = await supabase
        .from('profiles').select('*').eq('id', slug).single()

      if (byId) {
        setProf(byId)
        await loadServicos(byId.id)
        setLoading(false)
        return
      }

      // Tenta por slug do nome
      const { data: todos } = await supabase.from('profiles').select('*')
      const found = (todos || []).find((p: any) => {
        const s = (p.nome || '').toLowerCase()
  ...
  async function loadServicos(uid: string) {
    const { data } = await supabase.from('servicos').select('*').eq('profissional_id', uid)
    setServicos(data || [])
  }

  async function loadOcupados(date: Date) {
    const d = date.toISOString().split('T')[0]
    const { data } = await supabase.from('agendamentos').select('horario')
      .eq('profissional_id', prof.id).eq('data', d)
    setOcupados((data || []).map((a: any) => a.horario?.slice(0, 5)))
  }

  function selectDate(d: Date) {
    setSelDate(d); setSelTime(''); loadOcupados(d); setStep(3)
  }
  ...
  if (notFound) return (
    <div style={{ display:'flex', height:'100vh', alignItems:'center', justifyContent:'center', fontFamily:'system-ui', background:'#f2f1ec' }}>
      <div style={{ textAlign:'center' }}>
        ...
        <div style={{ fontSize:13, color:'#6b7280', marginTop:6 }}>Link: {slug}</div>
      </div>
    </div>
  )
    ...
    <div style={{ minHeight:'100vh', background:'#f2f1ec', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'system-ui', padding:16 }}>
      <div style={{ background:'#fff', borderRadius:20, padding:'32px 28px', maxWidth:400, width:'100%', textAlign:'center', boxShadow:'0 4px 24px rgba(0,0,0,0.08)' }}>
        ...
        <div style={{ fontSize:14, color:'#6b7280', lineHeight:1.6, marginBottom:20 }}>
          Seu horário com <strong>{prof?.nome}</strong> foi confirmado.
        </div>
        ...
        <div style={{ background:'#fff', borderRadius:20, padding:'24px 22px', boxShadow:'0 4px 24px rgba(0,0,0,0.08)' }}>
          ...
          {step===1 && (
            <div>
              ...
              {(servicos.length===0
                ? ['Manicure','Pedicure','Corte','Barba','Massagem','Consulta','Outro'].map(s=>({id:s,nome:s,preco:'',duracao:''}))
                : servicos
              ).map((s:any) => (
                <div key={s.id||s.nome} onClick={()=>{ setSelSvc(s); setStep(2) }}
                  style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'13px 16px', border:'1.5px solid #e5e7eb', borderRadius:12, cursor:'pointer', marginBottom:8 }}>
                  <div>
