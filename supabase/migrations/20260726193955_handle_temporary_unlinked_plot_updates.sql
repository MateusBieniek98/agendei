...
begin
  ...
  -- RPCs antigos atualizam projeto e texto, mas nao recebem talhao_id. Se a
  -- localizacao mudou, descarte o ID anterior antes de tentar canonizar.
  if tg_op = 'UPDATE'
     and (new.projeto_id is distinct from old.projeto_id
          or new.talhao is distinct from old.talhao)
     and new.talhao_id is not distinct from old.talhao_id then
    new.talhao_id := null;
  end if;
